\Chapter{Synthesis of Decoupled Computational Pipeline on FPGAs}{(\textit{previously published}~\cite{7082758}\cite{DBLP:journals/corr/ChengW16})}
\label{decoupleChap}
...
an extra push/pop operation is introduced into the set of instructions, associated
...
\begin{figure}[htp]
\begin{center}
\includegraphics[width=0.9\linewidth]{chap3fig/totalflow.pdf}
...
\begin{figure}[htp]
\begin{center}
\includegraphics[width=0.8\linewidth]{chap3fig/tili.pdf}
\caption{Transformation of a Function in LLVM IR 
\label{fig:tili}}
\end{center}
\end{figure}
...
of doing any computation. Depending on the actual structure of the control flow graph, some
...
More specifically, let the set of all basic blocks be $BB_{all}$, the set of basic blocks containing pop instructions (other than receiver of branch tags) be $BB_{comm}$ and the set of basic blocks performing some computation
...
\label{dupvscomm}
A major overhead our flow introduces comes from the mechanisms added
for different subgraphs to communicate with each other. In hardware accelerators,
these are manifested as resource overhead in implementing the extra push and
pop operations in each subgraph and more importantly, the FIFOs
between different subgraphs. 
%Table~\ref{tab:fifoResource} shows the resource usage for some FIFO configurations implemented on our experiment platforms. 
Given how even a minimal depth FIFO 
...
\begin{comment}
\begin{table}[htbp]
\caption{FPGA Resource Usage for Implementing FIFOs}
\centering
\begin{tabular}{| c | c | c | c | }
  \hline            
 \cline{3-4} 
  FIFO Dimension     & Logic(Slices) & BRAM & DSP    \\
  %
...
With the CPLEX~\cite{iILO06a} optimization software, the formulated ILP can be solved for each subgraph obtained. The experimental evaluation presented in section~\ref{sec:er} has incorporated the outcome of these optimizations.
...
access, consumers of the returned data are always in other downstream subgraphs.
%directly routed 
...
are incorporated into the final performance results shown in section~\ref{sec:er}.
...
\begin{table}[htbp]
\caption{Resource Usage of Decoupled Computational Pipelines and Conventional Accelerators.}
\centering
\begin{tabular}{| c | c | c | c | c | c | c | c| c | c | }
  \hline            
  \multirow{2}{*}{} &  &   \multicolumn{4}{c|}{ACP  } & \multicolumn{4}{c|}{ ACP + 64KB Cache }   \\
 \cline{3-10} 
 {Benchmark}   &    & LUT& FFs& BRAM &  DSP & LUT &	 FFs    	& BRAM &DSP    \\
  %
  %                 & LUT &FFs&  BRAM &  LUT &	 FFs    	& BRAM   \\
  \hline            
  \hline            
\multirow{3}{*}{}&Con.ACC  & 9873 &9116 &10 &5 & 7918 & 6792 & 21&5  \\
\cline{2-10}                                                                                                                                                    
SpMV &DCP       & 8577 & 8837& 10 &5& 6718  &6788 & 21&5\\
\cline{2-10}                                                                                                             
    Multiply   &\% change & -13.1 &-3.1 & 0&0 & -15.2  & -0.1 & 0&0  \\
  \hline                                                                                                           
\multirow{3}{*}{}&Con.ACC  & 7672 &7490 &8 &6 & 6573 & 5885 & 21 & 6\\
\cline{2-10}                                                                                                                                                    
Knapsack &DCP       & 8089 & 8787& 8& 6 & 6970  &7256 & 21&6 \\
\cline{2-10}                                                                                                             
       &\% change & +5.4 &+17.3 & 0 &0& +6.0  & +23.3 & 0& 0 \\
  \hline                                                                                                           
\multirow{3}{*}{}&Con.ACC  & 2491 &3528 &0&10 &3806  &4629  &19& 10  \\
\cline{2-10}                                                                                                                                                    
Floyd- &DCP       & 7659 &7210 &0 &10 &8995  &8309 &19&10 \\
\cline{2-10}                                                                                                             
 Warshall      &\% change &+207.5  &+104.3 & 0 &0 & +104.4  & +79.5  & 0& 0  \\
  \hline                                                                                                           
\multirow{3}{*}{}&Con.ACC  & 4810 &4929 &4& 0& 4931 & 4594 & 21& 0 \\
\cline{2-10}                                                                                                                                                    
DFS &DCP       & 8509 & 7813& 4 &0 &7436  &6298 & 21&0\\
\cline{2-10}                                                                                                             
...
The experimental results have validated the approach we have proposed and implemented. 
...
One possible direction for future exploration is to generalize our approach by incorporating, as a parameter, the set
...
One of the difficulties lies in modeling the performance gain of decoupling two nodes which sometimes also depends on how other parts of the CDFG interact with each other. It may not be realistic to capture all this information with a static graph, which is a
natural representation of computation and can be easily annotated with weights to express cost of partitioning instructions. 


On the other hand, if a quick estimator can be devised to evaluate the final performance from an instruction partitioning, randomized algorithms like simulated annealing may help direct the search towards a good solution. The challenge of
quickly quantifying the benefit of separating two nodes into independently running pipeline stages. 

To this end, a few quantities might be precomputed or profiled to assist the performance estimation for the generated pipeline. 
 In addition, off-chip communication bandwidth imposes an upper bound on how 


and the less  locality it has with respect to other instructions, the more beneficial it is to have it decoupled from other part of the CDFG. For our motivating example, 

quantities like the initiation intervals 
corresponding to each strongly connected component, might be precomputed

To this end, we can precompute the initiation intervals corresponding to each strongly connected components in the graph and profile the access pattern of each load/store node to estimate their
cache miss rate for a particular cache configuration. With this information, when presented with a particular instruction partitioning, 
we can quickly 



A more general framework, the development of which is left for future work, involves exploring the performance benefit v.s. area overhead by gradually
increase the number of subgraphs generated. 


The interaction between the memory accessing parts of the CDFG and the memory subsystem is hard to model with
